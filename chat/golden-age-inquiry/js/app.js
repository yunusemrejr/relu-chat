const SUGGESTIONS = [
  "Who was Al-Khwarizmi?",
  "What was the House of Wisdom?",
  "How did Ibn al-Haytham discover optics?",
  "Explain the Tusi couple",
  "What is the Canon of Medicine?",
  "Tell me about the astrolabe",
  "How did paper spread to Europe?",
  "Who discovered pulmonary circulation?"
];

const WELCOME =
  '<span style="font-size:1.125rem;font-weight:600;color:var(--text-primary);letter-spacing:-0.02em;">Golden Age Inquiry</span><br><br>Hi! I run <strong>entirely on-device</strong> — no servers, no API calls, no tracking. <br><br><strong>How it works:</strong> Your question is embedded with a transformer (progressive: heuristic fallback for instant first turns, hot-swaps to full MiniLM once loaded), then a signal layer combines BM25 sparse retrieval, entity extraction, and ensemble dense-sparse ranking (with top-k) into a calibrated decision packet. An <strong>RL-trained MLP policy network</strong> (13K params) decides mode, intent, tone, and topic composition. Responses are assembled from knowledge fragments with linguistic connectors and cross-fragment dedup — all in your browser. Session EMA gives multi-turn context.<br><br>First query warms up the model (~22 MB, cached + memoized). <a href="/how-it-works.html" style="color:var(--accent-light)">Read the architecture →</a>';

const botProfile = {
  id: "golden-age-inquiry",
  name: "Golden Age Inquiry",
  allowedIntents: ["definition", "example", "formal", "application", "comparison", "greeting", "help"],
  tone: "neutral",
  maxTopics: 3,
  creativityCeiling: 0.35
};

// Render UI shell immediately, then load heavy ML modules
import { pushMessage, setStatus } from '../../../core/ui.js';

pushMessage('bot', WELCOME);
setStatus('loading model...');

const [{ KB, entryText }, { INTENTS }, { CONFIG }, { overrides }, { createChatbot }] = await Promise.all([
  import('../../../data/bots/golden-age-inquiry/knowledge.js'),
  import('../../../data/bots/golden-age-inquiry/intents.js'),
  import('./config.js'),
  import('../../../data/bots/golden-age-inquiry/overrides.js'),
  import('../../../core/chatbot-engine.js'),
]);

createChatbot({
  KB,
  entryText,
  CONFIG,
  INTENTS,
  overrides,
  suggestions: SUGGESTIONS,
  welcomeMessage: null,
  botProfile
});
