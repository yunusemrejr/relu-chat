const SUGGESTIONS = [
  "What is Nash equilibrium?",
  "Example of a zero-sum game",
  "Formal definition of Shapley value",
  "Explain evolutionarily stable strategies",
  "Applications of mechanism design",
  "Prisoner's dilemma explained",
  "Minimax theorem",
  "Rubinstein bargaining"
];

const WELCOME = '<div style="font-size:1.125rem;font-weight:600;color:var(--text-primary);letter-spacing:-0.02em;margin-bottom:0.6em;">Game Theory Chat</div>Hi! I run <strong>entirely on-device</strong> — no servers, no API calls, no tracking.<br><br><strong>How it works:</strong> Your question is embedded with a transformer, then a signal layer combines BM25 sparse retrieval, entity extraction, and ensemble dense-sparse ranking into a calibrated decision packet. An <strong>RL-trained MLP policy network</strong> (13K params) decides mode, intent, tone, and topic composition. Responses are assembled from knowledge fragments with linguistic connectors and cross-fragment dedup — all in your browser. Session EMA gives multi-turn context. <strong>Topic correction:</strong> If I go off-track, say "I asked about X" or "no, just X" and I\'ll refocus.<br><br>First query warms up the model (~22 MB, cached). <a href="/how-it-works.html" style="color:var(--accent)">Read the architecture →</a>';

const botProfile = {
  id: "game-theory",
  name: "Game Theory Chat",
  allowedIntents: ["definition", "example", "formal", "application", "comparison", "greeting", "help"],
  tone: "neutral",
  maxTopics: 3,
  creativityCeiling: 0.35
};

// Render UI shell immediately, then load heavy ML modules
import { pushMessage, setStatus, escapeHTML, md } from '../../../core/ui.js';

pushMessage('bot', WELCOME);
setStatus('loading model...');

const [{ KB, entryText }, { INTENTS }, { CONFIG }, { overrides }, { createChatbot }] = await Promise.all([
  import('../../../data/bots/game-theory-chat/knowledge.js'),
  import('../../../data/bots/game-theory-chat/intents.js'),
  import('./config.js'),
  import('../../../data/bots/game-theory-chat/overrides.js'),
  import('../../../core/chatbot-engine.js'),
]);

createChatbot({
  KB,
  entryText,
  CONFIG,
  INTENTS,
  overrides,
  suggestions: SUGGESTIONS,
  welcomeMessage: null,
  botProfile
});
