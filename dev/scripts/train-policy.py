#!/usr/bin/env python3
"""
DEPRECATED: This training script is superseded by train-policy-unified.py.
Use: python3 dev/scripts/train-policy-unified.py --bot <bot-id>
...
# Heavy ML imports guarded so --skip-train / --help / pipeline validation work
# without torch installed (local dev laptop, pre-pip CI steps, etc).
# Full training requires: pip install -r dev/scripts/requirements.txt
import sys as _sys
if any(a in _sys.argv for a in ("--skip-train", "--help", "-h")):
    torch = None
    F = None
    np = None
    PolicyNetwork = None
    ACTION_SIZES_ORDERED = None
    N_FEATURES = None
else:
    import torch
...
# Provide arch constants even in --skip-train / no-torch mode (used by prompt/dataset builders for validation).
# Matches policy_model.py and policy/feature-extractor.js exactly.
if N_FEATURES is None:
    N_FEATURES = 25
if ACTION_SIZES_ORDERED is None:
    ACTION_SIZES_ORDERED = [("mode", 5), ("intent", 5), ("topic_count", 4), ("frag_count", 4), ("creativity", 1), ("tone", 4)]
...
def set_seed(seed: int):
    ...
    if np is None or torch is None:
        print(f"[seed] Skipped (no torch/np in this mode; --skip-train or missing deps)")
        return
    random.seed(seed)
    np.random.seed(seed)
...
def main():
    ...
    if args.val_split < 0 or args.val_split >= 1:
        raise SystemExit(
            f"[init] Invalid --val-split={args.val_split}; must satisfy 0 <= val-split < 1"
        )
    ...
    if args.val_split > 0 and args.val_split < 1:
        if len(dataset) < 2:
            print(f"[dataset] Too few samples ({len(dataset)}) for val split {args.val_split}; using all samples for training")
    ...
    if len(train_dataset) == 0:
        raise SystemExit("[dataset] Empty training set after splitting. Aborting to avoid invalid training loop.")

    if len(dataset) < 3:
        print(f"[dataset] Very small dataset ({len(dataset)} samples); val split/metrics may be noisy")
